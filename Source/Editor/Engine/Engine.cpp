...
void Engine::Start()
{
	...
	CameraManager::GetInstance().Start(mainWindow->GetSize(), lampShader, woodenBoxMaterial.GetMaterialShader());
...
void Engine::ApplyShader()
{
	...
	woodenBoxMaterial.SetCameraPosition(CameraManager::GetInstance().GetPosition());
	...
	woodenBoxMaterial.GetMaterialShader().SetVec3("light.position", _camera.GetPosition());
	woodenBoxMaterial.GetMaterialShader().SetVec3("light.direction", _camera.GetForward());
